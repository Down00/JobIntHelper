...
import { ref as dbRef, get, onValue, push, ref, remove, set, update } from 'firebase/database';
import { getDownloadURL, ref as storageRef, uploadBytesResumable } from 'firebase/storage';
import mime from 'mime';
import React, { useCallback, useEffect, useState } from 'react';
import { Alert, FlatList, Image, Modal, RefreshControl, ScrollView, StyleSheet, Switch, Text, TextInput, TouchableOpacity, View } from 'react-native';
...
import { auth, db, storage } from './../../../configs/FirebaseConfig';
    ...
    const fetchTrendingTopics = async (topics) => {
        const trendingTopics = topics
            .filter(topic => topic.likesCount > 0 || topic.totalCommentsAndReplies > 0)
            .sort((a, b) => (b.likesCount + b.totalCommentsAndReplies) - (a.likesCount + a.totalCommentsAndReplies))
        ...
        try {
            ...
            Object.keys(topicsData || {}).forEach(topicId => {
                ...
                if (!contributors[topic.userId]) {
                    contributors[topic.userId] = { postCount: 0, likeCount: 0, commentCount: 0 };
                }
            ...
            Object.keys(likesData || {}).forEach(topicId => {
                Object.keys(likesData[topicId] || {}).forEach(userId => {
                    if (!contributors[userId]) {
                        contributors[userId] = { postCount: 0, likeCount: 0, commentCount: 0 };
                    }
            ...
            Object.keys(commentsData || {}).forEach(topicId => {
                ...
                Object.keys(comments || {}).forEach(commentId => {
                    ...
                    if (!contributors[comment.userId]) {
                        contributors[comment.userId] = { postCount: 0, likeCount: 0, commentCount: 0 };
                    }
                    ...
                    if (comment.replies) {
                        Object.keys(comment.replies || {}).forEach(replyId => {
                            ...
                            if (!contributors[reply.userId]) {
                                contributors[reply.userId] = { postCount: 0, likeCount: 0, commentCount: 0 };
                            }
            ...
            const validContributors = enrichedContributors.filter(contributor => contributor !== null);
    ...
    export default function CommunityDiscussion() {
        ...
        useFocusEffect(
            useCallback(() => {
                const fetchUserNotifications = async () => {
                    ...
                    const enrichedNotifications = await Promise.all(data.map(async (notification) => {
        ...
        const fetchFavorites = async (topicId, userId) => {
            try {
                const favoriteSnapshot = await get(ref(db, `favorites/${topicId}/${userId}`));
    ...
    const fetchFavoritesCount = async (topicId) => {
        try {
            ...
            return favoritesSnapshot.exists() ? Object.keys(favoritesSnapshot.val()).length : 0;
        } catch (error) {
                    ...
                    if (userId !== topicOwnerId) {
                        ...
                        if (notificationSnapshot.exists()) {
                            ...
                            const notificationKey = Object.keys(notifications).find(
                                key => notifications[key].title === 'New Favorite on Your Topic' &&
                                notifications[key].body.includes(userName || 'Someone')
                            );
    
                            if (notificationKey) {
                                await remove(ref(db, `notifications/${topicOwnerId}/${notificationKey}`));
                ...
                if (topicSnapshot.exists()) {
                    ...
                    if (userId !== topicOwnerId) {
                        ...
                        if (ownerPushToken) {
                            ...
                            await sendPushNotification(ownerPushToken, notificationTitle, notificationBody, { topicId });
                            ...
                            console.log('Notification saved to Firebase for topic owner:', topicOwnerId);
                        }
                    }
                }
            }
    ...
    const fetchTopics = async () => {
        try {
            ...
            if (snapshot.exists()) {
                ...
                const topicList = await Promise.all(
                    topicIds.map(async (topicId) => {
                        ...
                        const { comments, totalCommentsAndReplies } = await fetchCommentsWithCount(topicId);
                        return {
                            ...topic,
                            ...reviewerData,
                            id: topicId,
            ...
            Alert.alert('Error', 'There was an error fetching topics. Please try again.');
        }
    };
        ...
        const otherTopics = filteredTopics.filter(
            (topic) => !trendingTopics.some((trendingTopic) => trendingTopic.id === topic.id)
        );
        ...
        const handleVote = async (topicId, selectedOption) => {
            try {
                ...
                const votedUsersRef = ref(db, `topics/${topicId}/poll/votedUsers/${userId}`);
                ...
                if (votedSnapshot.exists()) {
                    ...
                    if (votedOption === selectedOption) {
                        // User is unvoting
                        const pollOptionRef = ref(db, `topics/${topicId}/poll/options/${selectedOption}`);
                            ...
                            if (topicSnapshot.exists()) {
                                ...
                                if (topicOwnerId !== userId) {
                                    ...
                                    if (notificationSnapshot.exists()) {
                                        const notifications = notificationSnapshot.val();
                                        const notificationKey = Object.keys(notifications).find(
                                            key => notifications[key].title === 'New Vote on Your Poll' &&
                                            notifications[key].userId === userId // Match the userId who voted
                                        );
        
                                        if (notificationKey) {
                                            await remove(ref(db, `notifications/${topicOwnerId}/${notificationKey}`));
                                            console.log('Notification removed for poll owner:', topicOwnerId);
                                        }
                                    }
                                }
                            }
                ...
                if (snapshot.exists()) {
                    ...
                    const updatedTopics = Object.keys(data).map((topicId) => {
                        ...
                        return {
                            ...topic,
                            id: topicId,
                            profilePicture: topic.profilePicture || 'https://example.com/default-profile.png', // Fallback to default if missing
        ...
        const fetchTotalVotes = async (topicId) => {
            try {
                ...
                if (pollSnapshot.exists()) {
                    ...
                    const total = Object.values(options).reduce((sum, votes) => sum + votes, 0);
                    setTotalVotes(total);
                }
            } catch (error) {
        ...
        const handleCreateTopic = async () => {
            if (!newTopic.trim()) {
                Alert.alert('Notice📌', 'Please enter a topic before creating.', [{ text: 'OK' }]);
                return;
            }
            ...
            try {
                ...
                if (selectedImage) {
                    ...
                    const fileRef = storageRef(storage, `communityPosts/${userId}/${fileName}`);
                ...
                await set(ref(db, `topics/${newTopicKey}`), {
                    ...
                    timestamp: Date.now(),
                    imageUrl,
                    poll: isPoll ? pollOptions.reduce((acc, option) => {
                        acc[option] = 0;
                        return acc;
                    }, {}) : null,
                });
                ...
                Alert.alert("Error", "There was an error creating your topic. Please try again.");
            } finally {
                setLoading(false);
            }
        };
        ...
        const handleUpdateTopic = async (topicId) => {
            if (!editTopicText.trim() || (isEditingPoll && pollOptions.length < 2)) {
                Alert.alert('Notice📌', 'Please provide a topic and at least two poll options.', [{ text: 'OK' }]);
                return;
            }
            ...
            try {
                ...
                const existingPoll = pollSnapshot.exists() ? pollSnapshot.val() : { options: {}, votedUsers: {} };
                ...
                await update(ref(db, `topics/${topicId}/poll/votedUsers`), updatedVotedUsers);
                ...
                Alert.alert('Error', 'There was an error updating your poll. Please try again.');
            } finally {
                setLoading(false);
            }
        };
        ...
        const handleDeleteTopic = async (topicId, userId, isAdmin) => {
            try {
                ...
                if (topicSnapshot.exists()) {
                    ...
                    Alert.alert("Error", "You don't have permission to delete this topic.");
                    }
                } else {
            ...
            const submitReport = async () => {
                if (!reportReason.trim()) {
                    Alert.alert('Notice📌', 'Please provide a reason for reporting the topic.');
                    return;
                }
                    ...
                    if (topicSnapshot.exists()) {
                        ...
                        if (userId !== topicOwnerId) {
                            ...
                            if (notificationSnapshot.exists()) {
                                const notifications = notificationSnapshot.val();
                                const notificationKey = Object.keys(notifications).find(
                                    key => notifications[key].title === 'New Like on Your Topic' &&
        ...
        const pickImage = async () => {
            try {
                ...
                if (result.assets && result.assets.length > 0) {
                    ...
                    const userId = user.uid;
                    const fileName = `${Date.now()}.jpeg`;
                    const fileRef = storageRef(storage, `communityPosts/${userId}/${fileName}`); // Correct path for post images
            ...
            return Object.keys(options).map((option) => {
                ...
                return (
                    <View key={option} style={styles.pollOptionContainer}>
                        ...
                        <TouchableOpacity onPress={() => handleVote(topic.id, option)} style={styles.progressBarContainer}>
                            <ProgressBar progress={votePercentage / 100} color="#292929" style={styles.progressBar} />
                            <Text style={styles.progressBarText}>{Math.round(votePercentage)}%</Text>
                        </TouchableOpacity>
                    </View>
                );
            });
            ...
            <View style={[StyleSheet.absoluteFillObject, { backgroundColor: '#292929' }]} />
                    ...
                    <View style={styles.modalView}>
                        ...
                        {isPoll && (
                            <View style={styles.pollOptionsContainer}>
                                <Text style={styles.pollInstructions}>Enter at least two poll options:</Text>
                                        ...
                                        <View key={index} style={styles.pollOptionRow}>
                                            ...
                                            {index > 1 && (
                                                <TouchableOpacity onPress={() => handleRemovePollOption(index)} style={styles.removeOptionButton}>
                                                    <Text style={styles.removeOptionButtonText}>Remove</Text>
                                                </TouchableOpacity>
                                            )}
                                        </View>
                                    ))}
                                ...
                                <View style={styles.centerButtonWrapper}>
                                    <TouchableOpacity onPress={handleAddPollOption} style={styles.addPollOptionButton}>
                        ...
                        <View style={styles.modalButtons}>
                            <TouchableOpacity
                                ...
                                disabled={loading}
                            >
                                <Text style={styles.modalButtonText}>{loading ? 'Creating...' : 'Create Topic'}</Text>
                            </TouchableOpacity>
                ...
                <View style={styles.header}>
                    ...
                    <View style={styles.iconContainer}>
                        {/* Report data Icon */}
                        {isAdmin && (
                        <TouchableOpacity style={styles.icon} onPress={() => router.push('../admin-reports')}>
                ...
                >
                    ...
                    {favoriteTopics.length > 0 && (
                        <View>
                            ...
                            {favoriteTopics.slice(0, showAllFavorites ? favoriteTopics.length : 1).map((topic) => (
                                <View key={topic.id} style={styles.card}>
                                    <View style={styles.cardHeader}>
                                        <Image source={{ uri: topic.profilePicture }} style={styles.profilePicture} />
                                        <View style={styles.cardHeaderText}>
                                            <Text style={styles.userName}>{topic.userName}</Text>
                                            <Text style={styles.timestamp}>{formatTimestamp(topic.timestamp)}</Text>
                                        </View>
                                        {userId === topic.userId ? (
                                            ...
                                            <Menu>
                                                ...
                                                <MenuOptions>
                                                    ...
                                                    <MenuOption
                                                        onSelect={() => handleDeleteOption(topic.id, topic.userId)}  // Owner can delete their own topic
                                                        style={styles.menuOption}
                                                    >
                                            ...
                                            <Menu>
                                                ...
                                                <MenuOptions>
                                                    <MenuOption
                                                        onSelect={() => handleDeleteOption(topic.id, topic.userId)}  // Admin can delete any topic
                                                        style={styles.menuOption}
                                                    >
                                                        <Text style={styles.menuText}>Delete</Text>
                                                    </MenuOption>
                                        ...
                                        <View style={styles.editContainer}>
                                            ...
                                            {isEditingPoll && (
                                                <View>
                                                    ...
                                                    <TouchableOpacity onPress={handleAddPollOption} style={styles.addPollOptionButtonFavorites}>
                                                        <Text style={styles.addPollOptionText}>Add Another Option</Text>
                                                    </TouchableOpacity>
                                                </View>
                                            )}
                                            <View style={{ flexDirection: 'row' }}>
                                                <TouchableOpacity
                                                    ...
                                                    disabled={loading}
                                                >
                                                    <Text style={{ color: '#c1c1c1', fontSize: 16 }}>{loading ? 'Updating...' : 'Save'}</Text>
                                                </TouchableOpacity>
                                                <TouchableOpacity
                                                    style={styles.cancelButton}
                                                    onPress={handleCancelEdit}
                                                >
                                                    <Text style={{ color: '#c1c1c1', fontSize: 16 }}>Cancel</Text>
                                                </TouchableOpacity>
                                            </View>
                                        </View>
                                    ) : (
                                        <View>
                                            ...
                                            {topic.imageUrl && (
                                                <Image
                                                    ...
                                                    style={{ width: 200, height: 200, marginTop: 10, borderRadius: 15}}
                                                />
                                            )}
                                            {topic.poll && (
                                            ...
                                            <View style={styles.cardFooter}>
                                                <View style={styles.iconContainer}>
                                                    ...
                                                    >
                                                        ...
                                                        <Text style={styles.commentCount}>{topic.totalCommentsAndReplies}</Text>
                                                    </TouchableOpacity>
                                                </View>
                                                ...
                                                >
                                                    ...
                                                    />
                                                    <Text style={styles.favoriteCount}>{topic.favoritesCount}</Text>
                                                </TouchableOpacity>
                                            </View>
                                        </View>
                                    )}
                            ...
                            {favoriteTopics.length > 1 && !showAllFavorites && (
                                <TouchableOpacity onPress={() => setShowAllFavorites(true)}>
                                    <Text style={styles.viewMoreText}>
                                        View {favoriteTopics.length - 1} more {favoriteTopics.length - 1 === 1 ? 'favorite' : 'favorites'}
                                    </Text>
                                </TouchableOpacity>
                            )}
                                        ...
                                        <View style={styles.editContainer}>
                                            ...
                                            {isEditingPoll && (
                                                <View>
                                                    ...
                                                    <TouchableOpacity onPress={handleAddPollOption} style={styles.addPollOptionButtonTrending}>
                                                        <Text style={styles.addPollOptionText}>Add Another Option</Text>
                                                    </TouchableOpacity>
                                                </View>
                                            )}
                    ...
                    {otherTopics.length ? (
                                    ...
                                    <View style={styles.editContainer}>
                                        ...
                                        {isEditingPoll && (
                                            <View>
                                                ...
                                                <TouchableOpacity onPress={handleAddPollOption} style={styles.addPollOptionButtonOthers}>
                                                    <Text style={styles.addPollOptionText}>Add Another Option</Text>
                                                </TouchableOpacity>
                                            </View>
                                        )}
                        ...
                        <Text style={styles.noTopicsText}>No others topics yet. Be the first to add one!</Text>
                    )}
                                    ...
                                    <FlatList
                                        ...
                                        renderItem={({ item }) => (
                                            <View style={styles.likerItem}>
                                                <Image source={{ uri: item.profilePicture }} style={styles.likerImage} />
                                                <Text style={styles.likerName}>{item.userName || 'No Name'}</Text>
                                            </View>
                                        )}
                                    />
                                ) : (
                                    ...
                                    {notifications.length > 0 ? (
                                        notifications.map((notification, index) => (
                                            <View key={index} style={styles.notificationCard}>
                                                <Image
                                                    source={{ uri: notification.profilePicture || 'https://example.com/default-profile.png' }}
                                                ...
                                                <View style={styles.notificationTextContainer}>
                                                    <Text style={styles.notificationBody} numberOfLines={0}>
                                                        ...
                                                        {notification.body.replace(`${notification.userName || 'Someone'} `, '')}
                                                    </Text>
                                                ...
                                                <View key={index} style={styles.contributorCard}>
                                                    ...
                                                    <View style={styles.contributorTextContainer}>
                                                        ...
                                                        <Text style={styles.contributorStats}>
                                                            Posts: {contributor.postCount} | Likes: {contributor.likeCount} | Comments: {contributor.commentCount}
                                                        </Text>
                                                    </View>
                                                </View>
                                            );
